...
type Point = { _key: string; heading: string; text: string };
type Section = { _key: string; image: ImgRef; imageCaption: string; points: Point[] };
...
const ServicesPage = () => {
  ...
  const { data } = useQuery<Doc | null>({
    ...
    queryFn: () => sanityClient.fetch(`*[_id == "page-services"][0]`),
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-28 md:pt-32 pb-20 md:pb-32">
        <section className="container-wide text-center mb-16 md:mb-24">
          ...
          <motion.img {...fadeUp} transition={{ duration: 0.6, delay: 0.2 }} src={u(data.heroImage)} alt="Services" className="w-full rounded-xl" />
          <motion.p {...fadeUp} transition={{ duration: 0.6, delay: 0.25 }} className="text-muted-foreground text-sm italic mt-4">{data.heroCaption}</motion.p>
        </section>

        {data.sections.map((sec) => (
          <section key={sec._key} className="container-wide mb-16 md:mb-24">
            <div className="grid lg:grid-cols-2 gap-12 items-start">
              <motion.div {...fadeUp}>
                <img src={u(sec.image)} alt={sec.imageCaption} className="w-full rounded-xl" />
                <p className="text-muted-foreground text-sm italic mt-4">{sec.imageCaption}</p>
              </motion.div>
              <motion.div {...fadeUp} transition={{ duration: 0.6, delay: 0.1 }} className="space-y-6">
                {sec.points.map((pt) => (
                  <div key={pt._key} className="flex gap-4">
                    <span className="text-primary font-bold text-lg mt-0.5 shrink-0">•</span>
                    <p className="text-muted-foreground leading-relaxed">
                      <strong className="text-foreground">{pt.heading}</strong> {pt.text}
                    </p>
                  </div>
                ))}
              </motion.div>
